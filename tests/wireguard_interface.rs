use avena_overlay::{derive_wireguard_keypair, DeviceKeypair, EphemeralKeypair, HandshakeMessage};
...
fn three_node_interface_key_stability() {
    ...
    assert!(msg_to_b
        .verify(&local_device.public_key(), &peer_b.device_id())
        .is_ok());
    ...
    assert!(msg_to_c
        .verify(&local_device.public_key(), &peer_c.device_id())
        .is_ok());
