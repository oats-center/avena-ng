...
\begin{enumerate}
    ...
    \item Signature valid over: \texttt{ephemeral\_pubkey || nonce || timestamp || peer\_device\_id || wg\_pubkey}
...
The signature covers: \texttt{ephemeral\_pubkey || nonce || timestamp || peer\_device\_id || wg\_pubkey}.
...
\begin{enumerate}
    \item Generate interface name: \texttt{av-<hash8>} from hash of \texttt{(local\_id, peer\_id, underlay\_name)}
...
\textbf{AllowedIPs configuration:} All peer interfaces use \texttt{AllowedIPs = ::/0}. This permits Babel to route any destination through any peer. Babel's routing decisions, not WireGuard's AllowedIPs, determine which interface carries traffic.

If the peer is discovered over multiple underlay interfaces (e.g., WiFi and cellular), a separate Wireguard interface is created for each. Babel sees these as distinct links and can route traffic over whichever has better metrics, or failover automatically if one path degrades.
...
\textbf{Dead Peer Detection:} Wireguard maintains a last-handshake timestamp for each peer. If no successful handshake occurs within a configurable timeout (default: 180 seconds), the peer is considered potentially dead. Avena additionally sends lightweight keepalive probes over established tunnels.
