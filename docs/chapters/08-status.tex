...
At the time of writing, the following temporary implementation deltas remain relative to the target design in Chapter~2:

\begin{itemize}[nosep]
    \item \textbf{Babel startup behavior:} current code can continue with degraded behavior when \texttt{babeld} startup fails, while the design target states Babel is mandatory.
    \item \textbf{AllowedIPs policy:} current code uses an adaptive host-route strategy instead of universal \texttt{::/0} on all peer interfaces.
    \item \textbf{Interface model:} true per-(peer, underlay) WireGuard interface realization is not yet complete.
\end{itemize}
...
Wireguard backend & Dual-mode: Kernel backend via Linux netlink (\texttt{netlink-packet-wireguard} crate), userspace backend via \texttt{wireguard-go} subprocess with Unix socket UAPI. Host prefers kernel; sidecar uses userspace. \\
...
WireGuard interface model & One interface per (peer, underlay) tuple. Interface names: \texttt{av-<hash8>}. Enables true multi-path where Babel sees each physical path as distinct link. \\
