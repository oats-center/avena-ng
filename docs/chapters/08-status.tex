...
Dead Peer Detection & Configurable timeout using handshake recency plus receive-side traffic activity to avoid false liveness on transmit-only links & Complete \\
\midrule
...
Multi-path WireGuard & Per-(peer, underlay) interface creation with dual-underlay failover/churn scenario validation & Complete \\
...
Dual-underlay validation gates now pass (
\texttt{dual\_underlay\_same\_peer},
\texttt{dual\_underlay\_failover},
\texttt{dual\_underlay\_churn\_cleanup}
), including repeated teardown/rebuild cycles.
